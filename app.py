...
# ------------------------------------------------------------------------------------
# Custom component — full-resolution camera capture
# ------------------------------------------------------------------------------------
# Implemented as a static Streamlit component (plain HTML/JS, no build step, no
# external server). It uses the browser's native MediaDevices API to grab a
# still frame at the camera's full native resolution, avoiding the video
# compression artifacts that made streamlit-webrtc unsuitable for spotting
# small connector defects. See camera_component/index.html for the frontend.
_COMPONENT_DIR = Path(__file__).parent / "camera_component"
_camera_capture_component = components.declare_component(
    "camera_capture", path=str(_COMPONENT_DIR)
)


def camera_capture(key: str = "camera_capture"):
    """Renders the camera widget and returns a PIL.Image once the user
    presses Capture, or None while no image has been captured yet."""
    data_url = _camera_capture_component(key=key, default=None)
    if not data_url:
        return None
    try:
...
# ------------------------------------------------------------------------------------
# Image input — upload or camera capture
# ------------------------------------------------------------------------------------
...
with tab_camera:
    st.write(
        "Align the connector terminal in the live preview below, then press "
        "**Capture Image** to take a single still photo at the camera's full "
        "native resolution. Prediction only runs after you capture — nothing "
        "is analyzed continuously."
    )
    captured_image = camera_capture(key="connector_camera")
