...
def get_rtc_configuration() -> RTCConfiguration:
    """
    STUN alone often fails for users behind restrictive corporate/school
    firewalls (common on cloud deployments). If TURN credentials are present
    in st.secrets, add them as a fallback relay.

    Add this to your Streamlit Cloud app's Secrets (Settings > Secrets):
